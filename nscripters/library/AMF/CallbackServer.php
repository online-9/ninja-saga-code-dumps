<?php
    ...
    require_once __DIR__.'/AMF3/AcknowledgeMessage.php';
    require_once __DIR__.'/AMF3/RemotingMessage.php';
    require_once __DIR__.'/AMF3/CommandMessage.php';
    require_once __DIR__.'/AMF3/ErrorMessage.php';
     ...
     * @uses NScripters_Server
     * @uses NScripters_Message
     * @uses NScripters_Const
     */
    class NScripters_CallbackServer extends NScripters_Server {
         ...
         * 
         * @param NScripters_AMF3_CommandMessage $request
         * @return Sabre_AMF3_AbstractMessage 
         */
        private function handleCommandMessage(NScripters_AMF3_CommandMessage $request) {

            switch($request->operation) {

                case NScripters_AMF3_CommandMessage::CLIENT_PING_OPERATION :
                    $response = new NScripters_AMF3_AcknowledgeMessage($request);
                    break;
                case NScripters_AMF3_CommandMessage::LOGIN_OPERATION :
                    ...
                    $response->body = true;
                    break;
                case NScripters_AMF3_CommandMessage::DISCONNECT_OPERATION :
                    $response = new NScripters_AMF3_AcknowledgeMessage($request);
                    break;
                default :
        ...
        public function exec() {
            ...
            foreach($this->getRequests() as $request) {
                ...
                try {
                    ...
                    if (is_array($request['data']) && isset($request['data'][0]) && $request['data'][0] instanceof NScripters_AMF3_AbstractMessage) {
                        $request['data'] = $request['data'][0];
                    }

                    // See if we are dealing with the AMF3 messaging system
                    if (is_object($request['data']) && $request['data'] instanceof NScripters_AMF3_AbstractMessage) {
            
                        $AMFVersion = 3;
                       
                        // See if we are dealing with a CommandMessage
                        if ($request['data'] instanceof NScripters_AMF3_CommandMessage) {
                        ...
                        if ($request['data'] instanceof NScripters_AMF3_RemotingMessage) {

                            // Yes
                            $response = new NScripters_AMF3_AcknowledgeMessage($request['data']);
                    ...
                    $status = NScripters_Const::R_RESULT;
                    ...
                    if ($e instanceof NScripters_DetailException) {
                        $detail = $e->getDetail();
                    } else {
                        $detail = '';
                    }
                    ...
                    switch($AMFVersion) {
                        ...
                        case NScripters_Const::AMF3 :
                            $response = new NScripters_AMF3_ErrorMessage($request['data']);
                    ...
                    $status = NScripters_Const::R_STATUS;
                }
